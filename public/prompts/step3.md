...
1. **핵심 이슈**: 구성원들이 가장 많이/강하게 이야기하는 핵심 이슈
   - 언급 빈도: 높음 (여러 명이 반복 언급)
   - 감정적 강도: 강함 ("정말 힘들다", "가장 큰 문제", "최고의 장점")
   - 파서 형식: `[무형_레버_핵심]`, `[유형_레버_핵심]`, `[행동_핵심]`, `[결과_핵심]`

2. **일반 요소**: 구성원들이 보통 수준으로 언급하는 요소  
   - 언급 빈도: 중간 (절반 정도가 언급)
   - 감정적 강도: 중간 ("개선되었으면", "괜찮은 편")
   - 파서 형식: `[무형_레버_일반]`, `[유형_레버_일반]`, `[행동_일반]`, `[결과_일반]`
...
[유형_레버] (긍정/부정) 시스템 요소 (개념: 이론명, 출처: 학자명, 분류: 학문분야)
...
- **메타데이터는 한 줄로 통합**: `(개념: ..., 출처: ..., 분류: ...)`
- **이모지나 추가 태그 사용 금지**: 오직 하나의 `[타입]`만 사용
- **출처 표기 방식**:
- 학자명은 한글로 표기 (예: 막스 베버, 프레드릭 허즈버그)
- 공저자는 "~와 ~" 형식 (예: 시보와 워커)
...
- **[무형_레버_핵심]**: 구성원들이 가장 강하게/반복적으로 언급하는 문화적 동인
- **[무형_레버_일반]**: 구성원들이 보통 수준으로 언급하는 문화적 동인
- **[무형_레버_언급]**: 구성원들이 가끔 언급하는 배경 문화적 동인
- **[유형_레버_핵심]**: 구성원들이 가장 강하게/반복적으로 언급하는 시스템
- **[유형_레버_일반]**: 구성원들이 보통 수준으로 언급하는 시스템
- **[유형_레버_언급]**: 구성원들이 가끔 언급하는 배경 시스템
- **[행동_핵심]**: 구성원들이 가장 강하게/반복적으로 언급하는 행동
- **[행동_일반]**: 구성원들이 보통 수준으로 언급하는 행동
- **[행동_언급]**: 구성원들이 가끔 언급하는 배경 행동
- **[결과_핵심]**: 구성원들이 가장 강하게/반복적으로 언급하는 결과
- **[결과_일반]**: 구성원들이 보통 수준으로 언급하는 결과
...
```
[무형_레버_핵심] (부정) 안정성 우선 문화 (개념: Risk Aversion Theory, 출처: Hofstede, 분류: 문화인류학)
[유형_레버_일반] (부정) 결정 시스템 (개념: Centralized Decision Making, 출처: Weber, 분류: 조직학)
[행동_일반] (긍정) 팀 단위 협력
[행동_언급] (중립) 개별 업무 집중
[결과_핵심] (부정) 혁신 저하
[결과_언급] (긍정) 업무 효율성 유지
[연결] [무형_레버_핵심] (부정) 안정성 우선 문화 → [유형_레버_일반] (부정) 결정 시스템 (직접)
[연결] [유형_레버_일반] (부정) 결정 시스템 → [행동_일반] (긍정) 팀 단위 협력 (직접)
...
- [ ] 구성원 인식 강도(_핵심/_일반/_언급)가 모든 요소에 표시되었는가?
